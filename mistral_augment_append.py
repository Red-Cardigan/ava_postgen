...
def create_interaction(sentence, post, aug_p, interactions, augment=True):
    ...
    interaction = {
        "instruction": sentence,  # Assuming 'sentence' contains the instruction
        "context": "",  # Assuming there is no context to add here
        "response": content
    }
...
file_exists = os.path.isfile('mistral_ava_training.jsonl')
...
while offset < count:
    ...
    with open('mistral_ava_training.jsonl', 'a' if file_exists else 'w') as f:
        for post in posts:
            interactions = []
            ...
            for interaction in interactions:
                # Convert the interaction dictionary to a JSON string and write it to the file
                f.write(json.dumps(interaction) + '\n')

                update_query = "UPDATE top_writer_posts SET appended_to_mistraltxt = TRUE WHERE id = %s"
